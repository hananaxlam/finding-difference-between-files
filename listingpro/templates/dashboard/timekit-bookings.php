<?php
	...
	global $listingpro_options;	$timekit_bookings_enable = $listingpro_options['timekit_bookings_enable'];	if($timekit_bookings_enable != 1){		wp_redirect(home_url('/'));	}
...
if ( isset( $_POST['timekit_nonce_field'] ) && wp_verify_nonce( $_POST['timekit_nonce_field'], 'timekit_nonce' ) ) {
	...
	if(isset($lptimekitwidget) && !empty($lptimekitwidget) && isset($timekitListing) && !empty($timekitListing) ){
		update_post_meta( $timekitListing, 'timekit_booking', $lptimekitwidget);
	}
