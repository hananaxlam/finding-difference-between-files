	...
	if ( ! function_exists( 'listingpro_post_confirmation' ) ) {
		...
		function listingpro_post_confirmation($post) {
			if (isset($_POST['publish']) && !empty($_POST['publish_post']) ) {	
				...
				if( strpos($mail_content, '%website_url')==false ){
					$mail_content = $mail_content.'%website_url';
					$website_url = '';
				}
				if( strpos($mail_content, '%listing_title')==false ){
					$mail_content = $mail_content.'%listing_title';
					$listing_title = '';
				}
				if( strpos($mail_content, '%listing_url')==false ){
					$mail_content = $mail_content.'%listing_url';
					$listing_url = '';
				}
									...
									<li>
										...
										<?php
											...
											?>
													...
													if( !empty($check_plan_credit) && $check_plan_credit=="1" ){
													?>
