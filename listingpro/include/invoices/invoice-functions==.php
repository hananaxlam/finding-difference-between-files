...
$output = '
	...
	//to admin
			...
			$formated_mail_content = sprintf( $mail_content,$website_url,$listing_title,$plan_name,$plan_price,$listing_url,$invoice, $website_name, $payment_method  );
	...
	// to user
			...
			$body = sprintf( $bod,$website_url,$listing_title,$plan_name,$plan_price, $listing_url, $invoice, $website_name, $payment_method  );
...
if ( !function_exists('generate_wire_invoice') ){
	...
	function generate_wire_invoice( $postid ){
		...
		/* if changed plan */
		$changed_planid = listing_get_metabox_by_ID('changed_planid', $postid);
		if(!empty($changed_planid)){
			//lp_add_changeplan_data_in_db($postid);
		}
		/* end if changed plan */
		...
		if(!empty($userrow)){
			if( is_array( $userrow ) && count( $userrow ) > 0 ){
					if(!empty($cPlanID)){
						...
						$payment_method = get_post_meta($cPlanID, 'plan_package_type', true);
	...
	$website_name = get_option('blogname');
	$admin_email = get_option( 'admin_email' );
	//$plan_price = $plan_price.$userrow[0]->currency;
...
/* ==============changed plan db data save for wire=================== */
if(!function_exists('lp_add_changeplan_data_in_db($postid)')){
	function lp_add_changeplan_data_in_db($postid){
		global $listingpro_options;
		if(!empty($postid)){
			$user_ID = get_current_user_id();
			$changed_planid = listing_get_metabox_by_ID('changed_planid', $postid);
			$plan_ID = $changed_planid;
			...
			lp_create_listings_orders_table();
