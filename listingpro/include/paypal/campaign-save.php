...
if(!function_exists('lp_save_campaign_data')){
	function lp_save_campaign_data($adID, $transactionID, $method, $token, $status, $price_packages, $lpTOtalprice = '', $listing_id=''){
		...
		$mail_content = str_replace('%campaign_packages','%1$s', $mail_content);
		$mail_content = str_replace('%listing_title','%2$s', $mail_content);
		$mail_content = str_replace('%listing_url','%3$s', $mail_content);
		$mail_content = str_replace('%author_name','%4$s', $mail_content);
		
		$formated_mail_content = '';
		$formated_mail_content = sprintf( $mail_content,$campaign_packages,$listing_title,$listing_url, $author_name);
		...
		$formated_mail_content = '';
		$formated_mail_content = sprintf( $mail_content,$campaign_packages,$listing_title,$listing_url);
