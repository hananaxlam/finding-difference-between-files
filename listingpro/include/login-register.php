	...
	if (!function_exists('ajax_login')) {
		function ajax_login(){
			...
			if($processLogin == true){
				...
				echo json_encode(array('loggedin'=>false, 'message'=>'<span class="alert alert-danger"><i class="fa fa-times" aria-hidden="true"></i>'.esc_html__('Please provide captcha information','listingpro').' </span>'));
			}
		...
		function ajax_register(){
			...
			if($processRegister == true){
				...
				if ( !$user_id && email_exists($user_email) == false && !empty($user_email) && !empty($user_name) && is_email($user_email)) {
					...
					$mail_content = str_replace('%user_email_register','%4$s', $mail_content);

					$formated_mail_content = sprintf( $mail_content,$website_url,$user_name,$random_password,$user_email  );
				...
				$note = '<span class="alert alert-danger"><i class="fa fa-times" aria-hidden="true"></i>'.esc_html__(' Please provide captcha information','listingpro').'</span>';
				$final = json_encode(array('register'=>false, 'message'=> $note, 'receptcha'=>$keyResponse, 'gString'=>$gCaptcha ));
			}
