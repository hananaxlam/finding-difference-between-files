	...
	if(!function_exists('ajax_review_submit')){
		function ajax_review_submit(){
				...
				if($processReview==true){
					...
					$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Please provide captcha information', 'listingpro'), 'error'=>'email', 'processReview'=>$processReview, 'keyResponse'=>$keyResponse));
				}
