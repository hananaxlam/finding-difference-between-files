 ...
 */
	...
	function listingpro_style() {
		...
		if(is_singular('listing') || (is_front_page()) ||  is_tax( 'listing-category' ) || is_tax( 'feature' ) || is_tax( 'location' ) || (
                is_search()
	...
	function listingpro_scripts() {
		...
		/* ==============start add by sajid ============ */
		if(is_tax( 'location' ) || (is_front_page()) || is_tax( 'listing-category' ) || is_tax( 'feature' ) || (
                is_search()
                && isset( $_GET['post_type'] )
                && $_GET['post_type'] == 'listing'
		) ){
...
/* calculate average rate for listing */
					...
					if ( is_singular('listing') ){
						...
						if($is_reviewcall==true){
							...
							if(get_post_status( $listing_id )!='publish'){
								$ratingRes = '<span class="no-review">'.esc_html__("Rating only enable on published listing", "listingpro").'</span>';
							}
							else{
				...
				foreach($recurringData as $data){
					...
					if(is_numeric($plan_time)){
						...
						if($daysDiff == $plan_time){
							...
							$mail_content = str_replace('%listing_title','%1$s', $mail_content);
							$mail_content = str_replace('%plan_title','%2$s', $mail_content);
							$mail_content = str_replace('%plan_price','%3$s', $mail_content);
							$mail_content = str_replace('%plan_duration','%4$s', $mail_content);
							$mail_content = str_replace('%notifybefore','%5$s', $mail_content);
							$formated_mail_content = sprintf( $mail_content,$listing_title,$plan_title,$plan_price,$plan_time,$lp_nofify );
							...
							$mail_content_admin = str_replace('%listing_title','%1$s', $mail_content_admin);
							$mail_content_admin = str_replace('%plan_title','%2$s', $mail_content_admin);
							$mail_content_admin = str_replace('%plan_price','%3$s', $mail_content_admin);
							$mail_content_admin = str_replace('%plan_duration','%4$s', $mail_content_admin);
							$mail_content_admin = str_replace('%notifybefore','%5$s', $mail_content_admin);
							$formated_mail_content_admin = sprintf( $mail_content_admin,$listing_title,$plan_title,$plan_price,$plan_time,$lp_nofify );
	...
	if (!function_exists('listingpro_get_term_openfields')) {
		function listingpro_get_term_openfields() {
